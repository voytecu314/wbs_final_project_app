...
def get_all_scores(r: redis.Redis) -> List[Dict]:
    ...
    return [{"rank": i+1, "name": name, "score": int(score)} 
...
def main():
    ...
    if st.session_state.points > 0:
        if st.button(
            ...
            st.success(
                ...
                f"✅ {round(st.session_state.points)} Punkte eingereicht!"
            )
    ...
    for entry in page_scores:
        rank_display = f"🥇 #{entry['rank']}" if entry['rank'] == 1 else \
                      f"🥈 #{entry['rank']}" if entry['rank'] == 2 else \
                      f"🥉 #{entry['rank']}" if entry['rank'] == 3 else \
                      f"#{entry['rank']}"
        table_data.append({
            "Rank": rank_display,
            "Player": entry['name'],
