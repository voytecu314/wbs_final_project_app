...
def render_quiz_tools():
    ...
    st.markdown("Testen Sie Ihr Wissen mit automatisch generierten Fragen basierend auf Ihren RAG-Dokumenten.")
    ...
    def generate_quiz_question(topic, difficulty):
        ...
        # 2) Cooldown check
        if not can_generate():
            remaining = int(COOLDOWN_SECONDS - (time.time() - st.session_state.last_generation_time))
            st.warning(f"Bitte warten Sie {remaining} Sekunden, bevor eine neue Frage generiert wird.")
        ...
        try:
            ...
            fallback_q = "Welche Schutzausrüstung ist beim Schleifen von Metall Pflicht?"
            fallback_opts = [
                "A) Schutzbrille",
                "B) Arbeits-Sandalen",
                "C) Sonnenhut",
        ...
        question_text = " ".join(question_lines).strip() or "Was ist eine Grundregel der Metalltechnik?"
    ...
    topic = st.selectbox(
        "Thema auswählen:",
        ["Werkzeuge", "Sicherheit", "Maschinen", "Schweißen", "Feilen", "Drehen", "Metallarten"]
    )
    ...
    if st.session_state.current_question:
        ...
        if st.button("Antwort bestätigen"):
            ...
            if st.session_state.user_answer:
                selected_letter = re.match(r"^([A-Da-d])", st.session_state.user_answer).group(1).upper()
            else:
    ...
    st.info(
        f"Level: {st.session_state.level} — XP: {st.session_state.xp} — Schwierigkeit: {st.session_state.difficulty}"
    )
