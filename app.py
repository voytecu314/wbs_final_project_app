...
with st.sidebar.expander("Challenges"):
    quiz_page = st.radio(
        "",
        ["Choose a quiz:", "Hand Signs Quiz", "Tools Quiz", "Workshop Simulation Quiz"],
        on_change=lambda: st.session_state.__setitem__("button_clicked", False),
        key="quiz_nav",
    )

    st.session_state.page = (
        quiz_page if not st.session_state.button_clicked else st.session_state.page
    )
...
elif st.session_state.page == "Workshop Simulation Quiz":
