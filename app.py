...
if "quiz_selection_index" not in st.session_state:
    st.session_state.quiz_selection_index = 0
...
/* Wrappers */
	...
	#header-wrapper {
        ...
        background-image: url(https://imgpx.com/en/09cWNevd7IYf.png);
...
div[data-testid="stCheckbox"]>label[data-baseweb="checkbox"]:nth-child(1)>div:first-child {
    background-color: #64748b !important;
}
div[data-testid="stCheckbox"]>label[data-baseweb="checkbox"]:nth-child(1)>div:first-child>div:first-child {
    background-image: url("""+f"{british_flag if st.session_state.get('language_toggle', False) else german_flag}"+""");
    background-size: cover;
}
</style>
""",
...
def button_effects(page_name):
    ...
    st.session_state.button_clicked = True
    st.session_state.quiz_nav = "Choose a quiz:"
        

# --- MAIN NAV ITEMS (no radio bubbles) ---
st.sidebar.button(translate("Home","Startseite"), on_click=button_effects, args=("Home",))
...
with st.sidebar.expander(
    ...
    quiz_page = st.radio(
        ...
        key="quiz_nav",
        index=st.session_state.quiz_selection_index,
        label_visibility="collapsed"
    )

    st.session_state.page = quiz_page if not st.session_state.button_clicked else st.session_state.page
    
    st.session_state.quiz_selection_index = quizes.index(quiz_page)

st.sidebar.button(translate("STATS","Statistiken"), on_click=button_effects, args=("STATS",))

toggle_label = (
    "English"
    if st.session_state.get("language_toggle", False)
    else "Deutsch"
)
toggle_value = st.session_state.get("language_toggle", False)

is_toggle = st.sidebar.toggle(toggle_label, value=toggle_value, key="language_toggle")
#st.sidebar.info(f"The toogle is {is_toggle}")
